...
class TestSyntheticEnv:
    ...
    def test_mem_updates(self):
        ...
        assert len(mems) > 1, "mem must not be static"

    def test_reward_sign(self):
        env_many = SyntheticFaaSEnv(pattern="spike", seed=7); env_many.reset(); env_many._warm=15; env_many._t=365
        env_few  = SyntheticFaaSEnv(pattern="spike", seed=7); env_few.reset();  env_few._warm=1;  env_few._t=365
        r_many = [env_many.step(2)[1] for _ in range(20)]
        r_few  = [env_few.step(2)[1]  for _ in range(20)]
